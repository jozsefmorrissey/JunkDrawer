...
int main() {
  ...
  for (i = dictionary_ct - 1; i >= 0; i--) {
    if (*((void **)dictionary[i]) != &dictionary[i]) {
		printf("%p == %p\n", *((void **)dictionary[i]), &dictionary[i]);
...
void free(void *ptr) {
  ...
  if (!ptr)
	...
	if((size_t)ptr >= (size_t)head && (size_t)ptr <= (size_t)getback(tail)){
...
void * combineForward(memoryMap * ptr){
	...
	sbrk(0);
	size_t nextSize = 0;
	size_t currentSize = (size_t)getback(ptr) - (size_t)ptr;

	if(next)
		nextSize = (size_t)getback(next) - (size_t)next;
