...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
	...
	//std::cout << "element: " << subroot->elements[first_larger_idx].value << "  ";
	//std::cout << "key: " << key << "  ";
	if(subroot->elements.size() == 0)
		return V();
		
	if(subroot->elements.size() > first_larger_idx && key == subroot->elements[first_larger_idx])
	{
		//std::cout << "return: " << subroot->elements[first_larger_idx].value << "  ";
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
	...
	new_right->elements = vector<DataPair> ( make_move_iterator(mid_elem_itr+1), make_move_iterator(new_left->elements.end()));
	new_right->children = vector<BTreeNode*> (make_move_iterator(mid_child_itr), make_move_iterator(new_left->children.end()));
	
	/*for(size_t i = 0; i <mid_elem_idx; i++)	
	{
		size_t old = i + mid_elem_idx + 1;
		new_right->elements[i] = child->elements[old];
		std::cout << new_right->elements[i].key << std::endl;
	}*/

	/*for(size_t i = 0; i <mid_child_idx; i++)	
	{
		size_t old = i + mid_child_idx;
		new_right->children[i] = child->children[old];
	}*/



	size_t idx = insertion_idx(parent->elements, child->elements[mid_elem_idx]);
	//insertWhile(idx, parent, child->elements[mid_elem_idx]);


	/*DataPair temp = child->elements[idx];
	child->elements[idx++] = child->elements[mid_elem_idx];

	while(idx < child->elements.size())
	{
		child->elements[mid_elem_idx] = temp;
		temp = child->elements[idx];
		child->elements[idx++] = child->elements[mid_elem_idx];
	}*/
	...
	/*childrenWhile(parent, idx + 1, right);
	BTreeNode* temp2 = child->children[idx + 1];
	child->children[++idx] = right;

	while(idx + 1<child->children.size())
	{
		right = temp2;
		temp2 = child->children[idx + 1];
		child->children[++idx] = right;
	}*/

	//std::cout << "here i am: " <<parent->children[child_idx +1]->elements[0].key << std::endl;
	//std::cout << "here i am: " <<parent->children[child_idx]->elements[0].key << std::endl;
}

/*void childrenWhile(BTreeNode* curr, size_t i, std::vector< DataPair > right)
{
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
	...
	if(first_larger_idx < subroot->elements.size() && pair == subroot->elements[first_larger_idx])
		return;
	if(subroot->is_leaf)
	{	//insertWhile(first_larger_idx, curr, pair);
		/*DataPair temp = curr->elements[first_larger_idx];
		curr->elements[first_larger_idx++] = pair;

		while(first_larger_idx < curr->elements.size())
		{
			DataPair moreTemp = temp;
			temp = curr->elements[first_larger_idx];
			curr->elements[first_larger_idx++] = moreTemp;
		}*/
...
template <class K, class V>
void BTree<K, V>::printTree(BTreeNode * poopy)
{
	//std::cout << "order: " << order << endl;
	if(poopy==NULL)
		return;
	for(size_t i = 0; i<poopy->elements.size(); i++)
		std::cout << poopy->elements[i].key << ", ";

	std::cout << std::endl;
		
	for(size_t i = 0; i<poopy->children.size(); i++)
		printTree(poopy->children[i]);


}

/*template <class K, class V>
void insertWhile(size_t index, BTreeNode * curr, DataPair& key)
{
	DataPair temp = curr->elements[index];
	curr->elements[index++] = key;



	while(index < curr->elements.size())
	{
		key = temp;
		temp = curr->elements[index];
		curr->elements[index++] = key;
	}
}*/
