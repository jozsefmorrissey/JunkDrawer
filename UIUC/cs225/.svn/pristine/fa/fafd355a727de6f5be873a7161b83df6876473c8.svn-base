...
{
	Node * finder = root;
	vector<T> * blah = new vector<T>();
	//root->lastElem->elem = finder->elem;
	root->boo = true;
	isOrdered(root, blah);	
	T tacos = blah->back();
	blah->pop_back();
...
template <typename T>
void BinaryTree<T>::isOrdered(Node * subRoot, vector<T> * blah) const
{
	if(subRoot == NULL)
		return;
	isOrdered(subRoot->left, blah);
	blah->push_back(subRoot->elem);
	isOrdered(subRoot->right, blah);
