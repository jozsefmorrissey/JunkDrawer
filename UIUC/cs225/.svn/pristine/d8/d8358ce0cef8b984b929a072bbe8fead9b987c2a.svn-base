...
HuffmanTree::TreeNode * HuffmanTree::removeSmallest( 
	...
	else
	{
		...
		smallest = merge < single ? mergeQueue.front() : singleQueue.front();
		merge < single ? mergeQueue.pop() : singleQueue.pop();
	}// your code!
	return smallest;
}
...
void HuffmanTree::buildTree( const vector<Frequency> & frequencies ) {
	...
	TreeNode * parent = removeSmallest(singleQueue, mergeQueue);
	...
	{	
		...
		Frequency pfreq(second->freq.getFrequency() + first->freq.getFrequency());
		parent = new TreeNode(pfreq);
		parent->left = first;
		parent->right = second;
		mergeQueue.push(parent);
	} 
...
HuffmanTree::TreeNode * HuffmanTree::readTree( BinaryFileReader & bfile ) {
	...
	if(bit == 1)
	{	
		Frequency lfreq((char)bfile.getNextByte(), 0);
		leaf = new TreeNode(lfreq);
		return leaf;
	}
	else
	{
