...
class KDTree
{
	...
	void select(int dim, int begin, int end, int median);
	int partition(int returnedPart, int begin, int right, int dim);
	void buildTree(int begin, int end, int dim);
	Point<Dim> findNearestRecurse(const Point<Dim> & query, int begin, int end, int dim)const;
