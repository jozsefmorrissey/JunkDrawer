...
LogfileParser::LogfileParser( const string & fname ) : whenVisitedTable( 256 ) 
{
    ...
    while( infile.good() ) {
         ...
         * vector as well.
         */
		string keyString = ll.customer + ll.url;//keyStringBuilder9000(ll.customer, ll.url);
							

		auto oldDate = whenVisitedTable[keyString];
		bool oldNews = pageVisitedTable.find(keyString);

			if(oldDate < ll.date)
			{
				//pageVisitedTab[keyString, true);
				//unsigned int idx = hashes::hash(keyString, whenVisitedTable.tableSize());
				whenVisitedTable[keyString] = ll.date;
			}

			if(!pageVisitedTable.keyExists(keyString))
			{
...
std::string LogfileParser::keyStringBuilder9000(const string & customer, const string & url)const
{
		int firstIdx = url.find("/");
		string destroyer = url.substr(firstIdx + 1);
		int intermediateIdx =  destroyer.find("/");
		destroyer = destroyer.substr(intermediateIdx + 1);
		int endIdx = destroyer.find("/");
		string parcer = url.substr(firstIdx, firstIdx + intermediateIdx + 2);
...
bool LogfileParser::hasVisited( const string & customer, const string & url ) const {
	...
	string keyString = keyStringBuilder9000(customer, url);
