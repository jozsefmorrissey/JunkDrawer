...
{ 
	...
	int dx = abs(xc-x);//xc > x ? xc - x: x - xc;
	int dy = abs(yc-y);//yc > y ? yc - y: y - yc;
	int d = dx + dy;
	if(dx + dy >=rad)
		return fade2;

	int d1numRed = d*(int)fade1.red;
	int d2numRed = d*(int)fade2.red;
	newb.red = (int)fade1.red - (int)floor(d1numRed/rad) + (int)floor(d2numRed/rad);	

	int d1numBlue = d*(int)fade1.blue;
	int d2numBlue = d*(int)fade2.blue;
	newb.blue = (int)fade1.blue - (int)floor(d1numBlue/rad) + (int)floor(d2numBlue/rad);	

	int d1numGreen = d*(int)fade1.green;
	int d2numGreen = d*(int)fade2.green;
	newb.green = (int)fade1.green - (int)floor(d1numGreen/rad) + (int)floor(d2numGreen/rad);	
