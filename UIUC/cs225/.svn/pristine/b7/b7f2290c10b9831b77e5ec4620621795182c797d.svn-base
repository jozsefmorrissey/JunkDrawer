...
bool AnagramFinder<Dict>::checkWord( const string & word, const string & test ) {
	...
	if(words.get_elems() != tests.get_elems())
		return false;

	if(words.tableSize() < tests.tableSize())
	{
		for(auto it = words.begin(); it != words.end(); it++ )
		{
			auto trooper = tests.find(it->first);
			if(trooper != it->second)
			{
				return false;
			}
		}	
	...
	else
	{
		...
		{
			auto trooper = words.find(it->first);
			if(trooper != it->second)
				return false;
		}	
	}
