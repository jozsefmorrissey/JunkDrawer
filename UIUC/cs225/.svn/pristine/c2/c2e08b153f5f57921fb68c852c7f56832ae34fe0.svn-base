...
class BTree
{
    ...
    void printTree(BTreeNode * poopy);
...
size_t insertion_idx(const std::vector< T >& elements, const C& key)
{
	if(elements.empty())
		return 0;

	size_t start = 0;
	size_t end = (size_t)elements.size() - 1;
	...
	while(start <= end)
	{
		...
		if(elements[midpoint] < key)
			start = midpoint + 1;
		else if(key < elements[midpoint])
		{
			if(midpoint != 0)
				end = midpoint - 1;
			else
