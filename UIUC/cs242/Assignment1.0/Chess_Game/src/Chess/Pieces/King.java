...
public class King extends Piece {
    ...
    public VectorMove getMoves(Board board){
        VectorTuple moves = Movement.kingsMoves(this.getLocation(), board);
        VectorMove out = Move.createMoves(this.getLocation(), moves, board);

        VectorMove castel = casteling(board);
    ...
    public VectorMove casteling(Board board) {
        if (this.getTurns() > 0)
            return null;
        ...
        if (rook1 != null && rook1.getTurns() == 0 && checkClear(rook1.getLocation(), location, board)) {
            out.add(castel(rook1.getLocation(), board, -1));
        }
        if (rook2 != null && rook2.getTurns() == 0 && checkClear(location, rook2.getLocation(), board)){
            out.add(castel(rook2.getLocation(), board, 1));
        }
        return out;
    }

    //Checks if the path between the king and a given rook is clear.
    ...
    public Move castel(Tuple rook, Board board, int offSet){
