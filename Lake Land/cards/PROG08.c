...
//          up to 5 players allowing for one turn in of up to 4
//          cards.
...
{   srand(time(NULL));
    ...
    int seed, end=0;
        ...
        {   ct1=ct2=ct3=ct4=ct5=0;
            ...
            printf("Do you want to use the assigned function, play poker or ");
            printf("end program(A/P/E)?");
            prog=getchar();
            prog=getchar();
...
{   char cont;
    ...
    {   displayHand();
        ...
        {   //Skips option to continue when the deck is cashed out.
            ...
            //Introducing couter 5 allows the loop to keep running until a valid
            //input is received.
...
{   char again;
    ...
    {   ct1=ct2=ct3=ct4=ct5=0;
        ...
        //Shuffles bridges and displays the order for the number of times
        //specified.
        for(ct5=0; ct5<numShuf; ct5++)
        {   if(ct5==numShuf-1)  //Prints above the final order to declare the
            printf("\n\nThe final order is:\n");   //final card order.
            shuffle();
            bridge();
            displayOrder();
...
{   int trade, discard[4], initial;
    ...
    {   do
        {
            scanf("%i", &discard[ct2]);
            if(discard[ct2]<6 && discard[ct2]>0)
        {//Initial is the initial card location ct1=player-1 since array starts
         //at zero, one must be subtracted form the input values by the player.
            initial=5*(ct1)+discard[ct2]-1;

         //ct5 equals 25 initially and is incremented each time it is called.
            value[initial]=value[ct5];
            suit[initial]=suit[ct5++];
        }
        }while(discard[ct2]>5 || discard[ct2]<1);
    }
...
void shuffle (void)
{   int x;
    //Randomly decides which shuffle group to use.
    x=rand()%2;
    //Creates temporary copies of the arrays.
    for (ct1=0;ct1<52;ct1++)
    {   temp1[ct1]=value[ct1];
        temp2[ct1]=suit[ct1];
    ...
    {   ct2=51;
        ct3=0;
        for(ct1=0;ct1<13;ct1++)         //Each of these take four variables
        {  value[ct3++]=temp1[ct2-3];   //assigned to the bottom and places them
            value[ct3++]=temp1[ct2-2];  //on top.
            value[ct3++]=temp1[ct2-1];
            value[ct3++]=temp1[ct2];
            ct2-=4;
        }
        ct2=51;
        ct3=0;                          //Could have placed them all in a single
        for(ct1=0;ct1<13;ct1++)         //for loop.  It seems easier to read and
        {   suit[ct3++]=temp2[ct2-3];   //understand this way.
    ...
    {   ct2=51;
        ct3=0;
        for(ct1=0; ct1<4; ct1++)        //Each of these takes thirteen variables
        {   value[ct3++]=temp1[ct2-12]; //assigned to the bottom
